...
#' Filter using Network/Community Slots in an `omic` Object
#'
...
#' @export
#' @name filter_netw
#' @aliases filter_netw,omic-method filter_netw,omics-method
setGeneric("filter_netw", function(object, ..., .ungroup = FALSE, .deselect = FALSE) standardGeneric("filter_netw"))

setMethod("filter_netw", "omic", function(object, ..., .ungroup = FALSE, .deselect = FALSE) {
  ...
  if(isTRUE(.ungroup)) new_obj <- ungroup_omic(new_obj)

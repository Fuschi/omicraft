...
setMethod("mutate_meta", "omic", function(object, ...) {
  ...
  if (miss_sample(object)) {
    cli::cli_abort(c("x" = "No samples available in the {.cls omic} object."))}
    ...
    tryCatch(
      ...
      error = function(err) {
        ...
        cli::cli_abort(
          ...
          class  = "omic_mutate_meta_assign_error",
          parent = err
        )})
...
setMethod("mutate_meta", "omics", function(object, ...) {
  ...
  if (miss_sample(object, .fmt = "any")) {
    cli::cli_abort(c("x" = "No samples available in at least one {.cls omic} object."))}
  ...
  for (i in seq_along(exprs)) {
    ...
    if (any(evars %in% c("abun", "rela", "norm"))) {
      ...
      meta_tbl <- long_abun %>%
        dplyr::left_join(meta_tbl, by = dplyr::join_by(omic,sample_id))
      if(length(taxa_evars) != 0){
        meta_tbl <- taxa(object, .collapse = TRUE) %>%
          dplyr::select(tidyselect::all_of(c("omic", "taxa_id", taxa_evars))) %>%
          dplyr::right_join(meta_tbl, by = dplyr::join_by(omic, taxa_id))
      }
      meta_tbl <- meta_tbl %>%
        dplyr::group_by(!!!meta_groups) %>%
        dplyr::mutate(!!!rlang::eval_tidy(exprs[i])) %>%
...
#------------------------------------------------------------------------------#
#------------------------------------------------------------------------------#
#' Modify and Augment `omic` Objects by Transforming the `taxa` Slot
#'
#' This function dynamically manipulates the `taxa` slot within `omic` or `omics` objects,
#' applying user-defined transformations. It leverages the full suite of `tidyverse` tools, particularly
#' `dplyr`, to enable powerful and flexible data transformations.
#'
#' @param object An `omic` or `omics` object.
...
#'            contained in the list.
#'            
#' @return Returns the `omic` or `omics` object with updated `taxa` slots reflecting the applied transformations.
#'         All other structures within the object remain unchanged, ensuring that only the targeted metadata is modified.
#'
#' @export
#' @aliases mutate_taxa,omic-method mutate_taxa,omics-method
  ...
  for (i in seq_along(exprs)) {
    ...
    if (length(meta_evars) > 0L && !is_abun_expr) {
      cli::cli_abort(
        ...
        class = "omic_mutate_taxa_taxa_without_abundance"
      )
    }
    ...
    tryCatch(
      ...
      error = function(err) {
        ...
        cli::cli_abort(
          ...
          class  = "omic_mutate_taxa_assign_error",
          parent = err
        )})
...
setMethod("mutate_taxa", "omics", function(object, ...) {
  ...
  if (miss_taxa(object, .fmt = "any")) {
    cli::cli_abort(c("x" = "No taxa available in at least one {.cls omic} object."))}
  ...
  for (i in seq_along(exprs)) {
    ...
    if (any(evars %in% c("abun", "rela", "norm"))) {
      # Abundance-related expression => join abundance first
      taxa_tbl <- long_abun %>%
        dplyr::left_join(taxa_tbl, by = dplyr::join_by(omic,taxa_id))
      if(length(meta_evars) != 0){
        taxa_tbl <- meta(object, .collapse = TRUE) %>%
          dplyr::select(tidyselect::all_of(c("omic", "sample_id", meta_evars))) %>%
          dplyr::right_join(taxa_tbl, by = dplyr::join_by(omic, taxa_id))
      }
      taxa_tbl <- taxa_tbl %>%
        dplyr::group_by(!!!taxa_groups) %>%
        dplyr::mutate(!!!rlang::eval_tidy(exprs[i])) %>%

...
long_abundance_join <- function(object, needed_keys) {
  ...
  if(inherits(object, "omic")){
    ...
    if (miss_sample(object) || miss_taxa(object)) {
      cli::cli_abort("No sample or taxa available in the {.cls omic} object.")}
  ...
  } else if (inherits(object, "omics")){
    
    if(miss_sample(object, "any") || miss_taxa(object, "any")) stop("Error: No sample or taxa available in at least one of the omic objects.")
    ...
    cli::cli_abort(c(
      ...
      "i" = "Expected {.cls omic} or {.cls omics}, got: {.cls {class(object)}}."
    ))

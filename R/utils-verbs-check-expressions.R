...
check_reserved_keywords <- function(expressions) {
  ...
  found <- unique(expr_names[expr_names %in% .OMICS_RESERVED_COLNAMES])
...
#'
#' This function analyzes expressions to identify any references to specific abundance-related keys within an `omic` object. 
...
#'        These expressions are meant to manipulate or assess the abundance data stored in an `omic` object.
#'
